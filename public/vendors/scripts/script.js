...
jQuery(document).ready(function () {
    ...
    // 1. Ambil path URL (contoh: "/dashboard/unit.html")
    const path = window.location.pathname;

    // 2. Loop setiap link di sidebar
    $("#accordion-menu a").each(function() {
        const href = $(this).attr("href");

        // 3. Logika IF: Jika path URL mengandung nilai href link tersebut
        if (path.includes(href)) {
            $(this).addClass("active");

            // Jika link berada di dalam submenu, buka parent-nya
            const parentUl = $(this).closest('.submenu');
            if (parentUl.length) {
                parentUl.show(); // Menampilkan submenu
                parentUl.closest('li').addClass('show'); // Menandai menu induk
            }
        } else {
